...
#ifdef CONFIG_USB_ANDROID_SAMSUNG_COMPOSITE
	/* set interface number dynamically based on product function sequence. */
...
static void composite_disconnect(struct usb_gadget *gadget)
{
	...
	printk(KERN_DEBUG "[composite_disconnect]mute_switch= %d\n",cdev->mute_switch);
...
composite_switch_work(struct work_struct *data)
{
	...
	printk(KERN_DEBUG "[composite_switch_work]config=0x%p\n",(void*)config);
