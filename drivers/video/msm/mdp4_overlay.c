...
#else
#if 1 /* QC patch 20110623 */
void unfill_black_screen(void)
{
	printk("praveen: un fill black screen\n");
	...
	*/
	...
	temp_src_format = 6;
	MDP_OUTP(MDP_BASE + 0x18000, temp_src_format);
	
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
	return;
}
EXPORT_SYMBOL(unfill_black_screen);


void fill_black_screen(void)
{
	/*Black color*/
	printk("praveen: fill black screen\n");
...
EXPORT_SYMBOL(fill_black_screen);
#else /* org */
void unfill_black_screen(void)
{
	uint32 temp_src_format;
...
#endif		// #if 1
#endif		// #ifdef CONFIG_FB_MSM_HDMI_3D
...
void mdp4_overlay_vg_adjust_src_xoffset(struct mdp4_overlay_pipe *pipe,
     char *vg_base)
{
     uint32 luma_off = 0, chroma_off = 0;
     uint32 src_xy = (pipe->src_y << 16) | pipe->src_x;
     ...
     case MDP_Y_CBCR_H2V2:
     case MDP_Y_CRCB_H2V2:
//     case MDP_Y_CRCB_H1V1:   // Qcom will update by patch
//     case MDP_Y_CBCR_H1V1:
     case MDP_Y_CRCB_H2V1:
     case MDP_Y_CBCR_H2V1:
             luma_off = pipe->src_x;
     ...
     outpdw(vg_base + 0x0010, pipe->srcp0_addr + luma_off);
     /* chroma component plane or  planar color 1 */
     outpdw(vg_base + 0x0014, pipe->srcp1_addr + chroma_off);
     /* planar color 2 */
     outpdw(vg_base + 0x0018, pipe->srcp2_addr + chroma_off);
}
...
void mdp4_overlay_vg_setup(struct mdp4_overlay_pipe *pipe)
{
	...
	outpdw(vg_base + 0x0018, pipe->srcp2_addr + chroma_offset);
      ...
      */
     if (pipe->src_x)
             mdp4_overlay_vg_adjust_src_xoffset(pipe, vg_base);
...
//	pipe->req_data.dpp.sharp_strength = 120;
	...
	struct msm_fb_data_type *mfd, uint32 perf_level, uint32 pclk_rate)
{
	...
	pr_debug("%s: LCDC Mode Downscale validation with MDP Core"
		" Clk rate\n", __func__);
	pr_info("%s: src_w %u, src_h %u, dst_w %u, dst_h %u\n", __func__,
		req->src_rect.w, req->src_rect.h, req->dst_rect.w,
		req->dst_rect.h);

	if(!req->dst_rect.w||!req->dst_rect.h)
	{
		pr_err("%s: Invalid value dst_rect (%u, %u)\n",
	...
	if (!mdp_clk_hz) {
		pr_err("mdp_perf_level2clk_rate returned 0,"
	...
	unsigned long *start, unsigned long *len, struct file **pp_file)
{
	...
	if (MAJOR(file->f_dentry->d_inode->i_rdev) == FB_MAJOR) {
		...
		{
			pr_err("%s: fail to get_fb_phys_info!\n", __func__);
			ret = -1;
		}
		else
		{
		...
		pr_err("%s: fail to get device number!\n", __func__);
		ret = -1;
	}
	if (ret)
...
int mdp4_overlay_play_wait(struct fb_info *info, struct msmfb_overlay_data *req)
{
	...
	if (pipe->mixer_num == MDP4_MIXER1) {
		mdp4_overlay_dtv_wait_for_ov(mfd);	
	}
	mutex_unlock(&mfd->dma->ov_mutex);
...
#if 0//def CONFIG_FB_MSM_HDMI_MSM_PANEL_HDCP_SUPPORT
extern unsigned int fill_enable;
extern unsigned int unfill_enable;
#endif
...
/* mdp planar format expects Cb in srcp1 and Cr in p2 */
	...
	if (pipe->mixer_num == MDP4_MIXER1) {
...
#ifdef CONFIG_FB_MSM_DTV
		{
			...
			if(fill_enable == 1)
			{
				fill_black_screen(pipe);
				fill_enable = 0;
			}
			if(unfill_enable == 1)
			{
				unfill_black_screen(pipe);
				unfill_enable = 0;
			}
