...
#define NFC_DEBUG  0// 1    // samsung modify for delay
		...
		size_t count, loff_t *offset)
{
	...
	pr_debug("%s : reading %zu bytes. irq=%s\n", __func__, count,gpio_get_value_cansleep(pn544_dev->irq_gpio)?"1":"0");
...
wait_irq :
	...
	if (count == 1)/*Normal mode*/
	{
	    ...
	    if (ret < 0) {
			...
			pr_err("%s: i2c_master_recv error returned %d\n", __func__, ret);
			return ret;
		}
	...
	else /*download mode*/
	{
	    ...
	    if (ret < 0) {
 			mutex_unlock(&pn544_dev->read_mutex);			
			pr_err("%s: i2c_master_recv error2 returned %d\n", __func__, ret);
			return ret;
		}
		...
		const struct i2c_device_id *id)
{
	...
	ret = request_threaded_irq/*request_irq*/(irq_num/*client->irq*/, NULL, pn544_dev_irq_handler,
