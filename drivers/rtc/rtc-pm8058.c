...
#define __WORKAROUND_FOR_COINCELL_FAST_DISCHARGING__ // workaround for conicell backup battery fast discharging

#ifdef __WORKAROUND_FOR_COINCELL_FAST_DISCHARGING__
...
pm8058_rtc0_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	pr_info("[%s] time set to"
			" %d-%02d-%02d %02d:%02d:%02d.%09lu UTC\n", __func__,
			tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday,
			tm->tm_hour, tm->tm_min, tm->tm_sec, secs);
...
static int __devinit pm8058_rtc_probe(struct platform_device *pdev)
{
...
#ifdef __WORKAROUND_FOR_COINCELL_FAST_DISCHARGING__
	rc = pm8058_rtc0_read_time(&pdev->dev, &rtc_tm);
	if (rc) {
		pr_err("%s: Unable to read RTC time\n", __func__);
		goto _exit;
	}
	...
	if(secs_rtc < EARLIEST_SUPPORTED_TIME) {
#ifdef CONFIG_RTC_PM8058_WRITE_ENABLE
		rtc_time_to_tm(EARLIEST_SUPPORTED_TIME, &rtc_tm);

		rc = pm8058_rtc0_set_time(&pdev->dev, &rtc_tm);
		if (rc) {
