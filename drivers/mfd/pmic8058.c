...
int pm8058_irq_get_it_status(struct pm8058_chip *chip, int irq)
{
	int     rc;
	u8      block, bits, bit;
	...
	rc = ssbi_read(chip->dev, SSBI_REG_ADDR_IRQ_IT_STATUS, &bits, 1);
	if (rc) {
...
EXPORT_SYMBOL(pm8058_irq_get_it_status);
#endif
...
// kmj_eh09
// SAMSUNG CODE
// When s/w reset is done, it should be done by only wdog but not by smpl
// so SMPLD should be off at every reset.
#define FEATURE_RESET_WITH_WDOG
...
pm8058_config_irq(struct pm8058_chip *chip, u8 *bp, u8 *cp)
{
	...
	// fix ADC
#ifdef CONFIG_SEC_DEBUG_PM8058_ADC_VERBOSE	
	if (++samsung_count > 150 && *bp == 9 && ((*cp & 0x70) == 0x40)) {
		printk(KERN_ERR "SAMSUNG: writing config=0x%x for ADC irq\n",*cp);
		...
		if ((bits & 0x10) && !(*cp & 0x08)) {
			pr_err("SAMSUNG %s: it_status=0x%xthe ADC interrupt is pending and will be lost\n", __func__, bits);
			__WARN();
		}
	}
#endif
...
#ifdef CONFIG_SEC_DEBUG_PM8058_ADC_VERBOSE	
		pr_err("%s: no need to enable an already enabled irq=%d\n",
...
#ifdef CONFIG_SEC_DEBUG_PM8058_ADC
	u8 block_sel; 
	static uint8_t pm8058_suspend_before, pm8058_suspend_after, 
		pm8058_suspend_IT_before, pm8058_suspend_RT_before, 
		pm8058_suspend_IT_after, pm8058_suspend_RT_after; 

	pm8058_read(chip, 0x197, &pm8058_suspend_before, 1); 
	block_sel = 9; 
	pm8058_write(chip, 0x1c0, &block_sel, 1); 
	pm8058_read(chip, 0x1c1, &pm8058_suspend_IT_before, 1); 
	pm8058_read(chip, 0x1c3, &pm8058_suspend_RT_before, 1); 
#endif
...
#ifdef CONFIG_SEC_DEBUG_PM8058_ADC
	pm8058_read(chip, 0x197, &pm8058_suspend_after, 1); 
	pm8058_write(chip, 0x1c0, &block_sel, 1); 
	pm8058_read(chip, 0x1c1, &pm8058_suspend_IT_after, 1); 
	pm8058_read(chip, 0x1c3, &pm8058_suspend_RT_after, 1); 
#endif
...
#ifdef CONFIG_SEC_DEBUG_PM8058_ADC
	u8 block_sel; 
	static uint8_t pm8058_resume_before, pm8058_resume_after, pm8058_resume_IT_before, 
		pm8058_resume_RT_before, pm8058_resume_IT_after, pm8058_resume_RT_after; 

	pm8058_read(chip, 0x197, &pm8058_resume_before, 1); 
	block_sel = 9; 
	pm8058_write(chip, 0x1c0, &block_sel, 1); 
	pm8058_read(chip, 0x1c1, &pm8058_resume_IT_before, 1); 
	pm8058_read(chip, 0x1c3, &pm8058_resume_RT_before, 1); 
#endif
...
#ifdef CONFIG_SEC_DEBUG_PM8058_ADC
	pm8058_read(chip, 0x197, &pm8058_resume_after, 1); 
	pm8058_write(chip, 0x1c0, &block_sel, 1); 
	pm8058_read(chip, 0x1c1, &pm8058_resume_IT_after, 1); 
	pm8058_read(chip, 0x1c3, &pm8058_resume_RT_after, 1); 
#endif
