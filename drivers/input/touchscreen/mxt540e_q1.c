...
#include "mXT540__APP_v1_0_FB_test_build_.h"
...
static int __devinit calculate_infoblock_crc(struct mxt540e_data *data,	u32 *crc_pointer)
{
	u32 crc = 0;
...
static int __devinit mxt540e_init_touch_driver(struct mxt540e_data *data)
{
	...
	dev_info(&data->client->dev, "family = %#02x, variant = %#02x, version " "= %#02x, build = %d\n", id[0], id[1], id[2], id[3]);
	printk(KERN_ERR"family = %#02x, variant = %#02x, version " "= %#02x, build = %d\n", id[0], id[1], id[2], id[3]);
	...
	ret = read_mem(data, OBJECT_TABLE_START_ADDRESS, data->objects_len * sizeof(*object_table),(u8 *)object_table);
	if (ret)
		goto err;
	...
	for (i = 0; i < data->objects_len; i++) {
		...
		if (object_table[i].num_report_ids) {
			tmp = type_count + 1;
			type_count += object_table[i].num_report_ids * (object_table[i].instances + 1);
		}
		...
		case GEN_MESSAGEPROCESSOR_T5:
			...
			dev_dbg(&data->client->dev, "Message object size = " "%d\n", data->msg_object_size);
			break;		
		}
	}
	...
	/* Verify CRC */
	crc_address = OBJECT_TABLE_START_ADDRESS + data->objects_len * OBJECT_TABLE_ELEMENT_SIZE;
...
#error The following code will likely break on a big endian machine
	...
	ret = get_object_info(data, GEN_MESSAGEPROCESSOR_T5, &dummy,&data->msg_proc);
	if (ret)
		goto err;

	ret = get_object_info(data, GEN_COMMANDPROCESSOR_T6, &dummy,&data->cmd_proc);
	if (ret)
		goto err;

	ret = get_object_info(data, SPT_MESSAGECOUNT_T44, &dummy, &data->t44_proc);
	if (ret)
		goto err;    
...
static void report_input_data(struct mxt540e_data *data)
{
	...
	for (i = 0; i < data->num_fingers; i++) {        
		...
		// input_report_abs(data->input_dev, ABS_MT_SUMSIZE, data->fingers[i].sum_size);
		...
		if (data->fingers[i].state == MXT540E_STATE_PRESS || data->fingers[i].state == MXT540E_STATE_RELEASE) {
			//printk(KERN_DEBUG "[TSP] id[%d],x=%d,y=%d\n", i , data->fingers[i].x, data->fingers[i].y );
			//printk(KERN_DEBUG "[TSP] id[%d],z=%d,w=%d\n", i , data->fingers[i].z, data->fingers[i].w );
		}
...
static irqreturn_t mxt540e_irq_thread(int irq, void *ptr)
{
	...
	int id;
	u8 msg[data->msg_object_size]; // sizeof 9 bytes
    ...
    read_mem(data, data->t44_proc, 1, &pending_message_count);
	...
	do {
		...
		if (msg[0] > 1 && msg[0] < 12) {
            ...
            //printk(KERN_DEBUG "BEFORE [MID_TSP] msg[0] = (0x%x), msg[1] = (0x%x), id = (%d), finger_mask = (%d)\n", msg[0], msg[1], id, data->finger_mask );
			/* If not a touch event, then keep going */
			if (id < 0 || id >= data->num_fingers){
					continue;
            }

           //printk(KERN_DEBUG "[TSP] data->finger_mask(0x%x), id = (%d)\n", data->finger_mask, id);
			...
			} else if ((msg[1] & DETECT_MSG_MASK) && (msg[1] & (PRESS_MSG_MASK | MOVE_MSG_MASK | VECTOR_MSG_MASK | AMP_MSG_MASK))) {
				touch_message_flag = 1;
				data->fingers[id].sum_size = sumsize;
...
static ssize_t mxt540e_debug_setting(struct device *dev, struct device_attribute *attr,	const char *buf, size_t count)
{
	g_debug_switch = !g_debug_switch;
	return 0;
}

static ssize_t mxt540e_object_setting(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t mxt540e_object_show(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct mxt540e_data *data = dev_get_drvdata(dev);
	unsigned int object_type;
	u8 val;
...
static u8 build_latest = 0xFB;      //0xAA;   //0xFB;

#if defined (CONFIG_KOR_MODEL_SHV_E160S) // For Kor's Factory test Mode 
...
#define MAX_VALUE 21224
#define MIN_VALUE 29884
...
static int mxt540e_check_bootloader(struct i2c_client *client, unsigned int state)
{
	u8 val;
	u8 temp;
...
static int mxt540e_fw_write(struct i2c_client *client, const u8 *data, unsigned int frame_size)
{
...
static int mxt540e_load_fw(struct device *dev, const char *fn)
{
    ...
    fw->size = ARRAY_SIZE(mXT540__APP_v1_0_FB_test_build);
    fw->data = mXT540__APP_v1_0_FB_test_build;
...
ssize_t disp_all_deltadata_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
...
static int __devinit mxt540e_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	for (i = 0; i < data->num_fingers; i++) {
 	...
 	ret = request_threaded_irq(client->irq, NULL, mxt540e_irq_thread, IRQF_TRIGGER_LOW | IRQF_ONESHOT, "mxt540e_ts", data);
	...
	if (IS_ERR(qt602240_noise_test))
		printk(KERN_ERR "[TSP] Failed to create device(qt602240_noise_test)!\n");
	...
	if (device_create_file(qt602240_noise_test, &dev_attr_set_threshold) < 0)
