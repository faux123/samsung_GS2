...
static int barometer_read_pressure_data(struct barometer_data *barom)
{
	...
	if (status < 0) {
		pr_err("%s: reading status reg failed\n", __func__);
		err = status;
		goto done;
	} else if ((status & 0x03) < 0x03) {
		pr_err("%s: new data is not available yet, skip data reading\n",
			__func__);
		err = -EIO;
		goto done;
	}
	...
	if (err < 0) {
		...
		barom->temperature = (s8)(temp_data[1] << 8) | temp_data[0];
				  ...
				  const char *buf, size_t size)
{
	...
	if (new_value) {
		...
		if (err < 0)
			pr_err("%s: barometer enable failed\n", __func__);
	} else {
		err = barometer_disable(barom);
		if (err < 0)
			pr_err("%s: barometer disable failed\n", __func__);
	}
	mutex_unlock(&barom->lock);
