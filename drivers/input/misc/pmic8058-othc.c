...
static int pm8058_accessory_report(struct pm8058_othc *dd, int status)
{
	...
	pr_info("pm8058_accessory_report\n");
	...
	pr_info("micbias_status;%d   switch_status :%d \n", micbias_status, switch_status);

	/* Loop through to check which accessory is connected */
	for (i = 0; i < dd->num_accessories; i++) {

    	pr_info("accessory_info[%d] enabled(%d) flag(%d) gpio(%d) active_low(%d)  \n",
            i, dd->accessory_info[i].enabled, dd->accessory_info[i].detect_flags,
                dd->accessory_info[i].gpio, dd->accessory_info[i].active_low );
	...
	if (detected) {
    	...
    	pr_info("[OTHC_HEADSET=%d OTHC_HEADPHONE=%d] curr_accessory :%d    curr_accessory_code :%d\n",OTHC_HEADSET, OTHC_HEADPHONE, dd->curr_accessory, dd->curr_accessory_code );
