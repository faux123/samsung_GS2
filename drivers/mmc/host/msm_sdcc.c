...
msmsdcc_dma_exec_func(struct msm_dmov_cmd *cmd)
{
	...
	if (readl(host->base + MMCISTATUS) & 0x3000) 
		printk(KERN_ERR "%s: TX/RXACTIVE is on at turning on DPSM MCI_STATUS:0x%08x in %s\n" ,mmc_hostname(host->mmc), readl(host->base + MMCISTATUS), __func__);
...
msmsdcc_dma_complete_tlet(unsigned long data)
{
	...
	if (host->curr.got_dataend || mrq->data->error) {
		...
		} else
		{
			...
			{
				int i;
				pr_info("%s: ====SDCC registers=== in %s\n", mmc_hostname(host->mmc), __func__);

				for (i = 0; i < 18; i++) {
					pr_info("Address = 0x%.4x \t Value = 0x%.8x\n",(i*4), readl(host->base + (i*4)));
				}
				pr_info("%s: =====================in %s \n", mmc_hostname(host->mmc), __func__);
			}
		}
...
msmsdcc_pio_irq(int irq, void *dev_id)
{
	...
	WARN(!host->clks_on, "[%s]%s MCI_STATUS:%x",__func__, mmc_hostname(host->mmc), status);
...
static void msmsdcc_req_tout_timer_hdlr(unsigned long data)
{
	...
	if ((25 == mrq->cmd->opcode || 18 == mrq->cmd->opcode) && (strcmp("mmc0", mmc_hostname(host->mmc)) == 0))
	{
		panic("MSM SDCC timeout");
	}
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	if (plat->status_irq) {
		...
		set_irq_wake(plat->status_irq, 1); //PMMC 20110720
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
        ...
        {
            msmsdcc_set_pwrsave(mmc, msmsdcc_is_pwrsave(host));
        }
#endif
...
//PMMC		disable_irq(host->plat->status_irq);
...
//PMMC	if (host->plat->status_irq) {
//PMMC		msmsdcc_check_status((unsigned long)host);
//PMMC		enable_irq(host->plat->status_irq);
//PMMC	}
