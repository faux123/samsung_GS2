...
#define CAM_DEBUG(fmt, arg...)	\
		do {\
			printk(KERN_DEBUG "[4ECGX] %s : " fmt "\n", __FUNCTION__, ##arg);}\
		while(0)
			
#define cam_info(fmt, arg...)	\
		do {\
		printk(KERN_INFO "[4ECGX]" fmt "\n",##arg);}\
		while(0)
			
#define cam_err(fmt, arg...)	\
		do {\
		printk(KERN_ERR "[4ECGX] %s:%d:" fmt "\n",__FUNCTION__, __LINE__, ##arg);}\
		while(0)
...
struct s5k4ecgx_setting_struct {
...
struct s5k4ecgx_ctrl_t {
	...
	struct s5k4ecgx_status_struct status;
	
	struct s5k4ecgx_setting_struct setting;
