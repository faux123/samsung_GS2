...
int fc8050_spi_write_then_read(struct spi_device *spi, u8 *txbuf, u16 tx_length, u8 *rxbuf, u16 rx_length)
{
	...
	x.tx_buf=txbuf;//data_buf;
	x.rx_buf=txbuf;//data_buf;
...
int fc8050_spi_write_then_burstread(struct spi_device *spi, u8 *txbuf, u16 tx_length, u8 *rxbuf, u16 rx_length)
{
	s32 res;
...
static int spi_dataread(HANDLE hDevice, u8 addr, u8* data, u16 length)
{
  ...
  fc8050_spi = spi_dmb;
	...
	//ret = fc8050_spi_write_then_read(fc8050_spi, &tx_data[0], 2, &data[0], length);
	ret = fc8050_spi_write_then_burstread(fc8050_spi, &tx_data[0], 2, &data[0], length);
