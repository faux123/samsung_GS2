 ...
 *
 * Additional code modifications for the msm8x60 architecture
 * for Qualcomm S3 SoCs by Paul Keith (javelinanddart)
 * Modifications Copyright (c) 2015
 *
 */
...
#define DEFAULT_SENSOR_ID			6
#define DEFAULT_POLLING_MS			500
#ifdef CONFIG_CPU_OVERCLOCK
#define DEFAULT_LIMIT_TEMP_DEGC		80
#else
#define DEFAULT_LIMIT_TEMP_DEGC		70
#endif
#define DEFAULT_TEMP_HYSTERESIS_DEGC 		10
#define DEFAULT_FREQ_STEP 			2
#define DEFAULT_FREQ_CONTROL_MASK		0xf
#ifdef CONFIG_CPU_OVERCLOCK
#define DEFAULT_CORE_LIMIT_TEMP_DEGC		85
#else
#define DEFAULT_CORE_LIMIT_TEMP_DEGC		75
#endif
#define DEFAULT_CORE_TEMP_HYSTERESIS_DEGC	10
#define DEFAULT_CORE_CONTROL_MASK		0xe
/* last 3 minutes based on 250ms polling cycle */
#define MAX_HISTORY_SZ		((3*60*1000) / DEFAULT_POLLING_MS)

uint32_t temp_history[MAX_HISTORY_SZ];
uint32_t overtemp;
uint32_t warning;
uint32_t normal;

static int enabled;
uint32_t sensor_id = DEFAULT_SENSOR_ID;
uint32_t poll_ms = DEFAULT_POLLING_MS;
uint32_t limit_temp_degC = DEFAULT_LIMIT_TEMP_DEGC;
uint32_t temp_hysteresis_degC = DEFAULT_TEMP_HYSTERESIS_DEGC;
uint32_t freq_step = DEFAULT_FREQ_STEP;
uint32_t freq_control_mask = DEFAULT_FREQ_CONTROL_MASK;
uint32_t core_limit_temp_degC = DEFAULT_CORE_LIMIT_TEMP_DEGC;
uint32_t core_temp_hysteresis_degC = DEFAULT_CORE_TEMP_HYSTERESIS_DEGC;
uint32_t core_control_mask = DEFAULT_CORE_CONTROL_MASK;
...
/* module parameters */
module_param_named(poll_ms, poll_ms, uint, 0664);
module_param_named(limit_temp_degC, limit_temp_degC,
			int, 0664);
module_param_named(temp_hysteresis_degC, temp_hysteresis_degC,
			int, 0664);
module_param_named(freq_control_mask, freq_control_mask,
			uint, 0664);
module_param_named(core_limit_temp_degC, core_limit_temp_degC,
			int, 0664);
module_param_named(core_temp_hysteresis_degC, core_temp_hysteresis_degC,
			int, 0664);
module_param_named(core_control_mask, core_control_mask,
			uint, 0664);
...
static void __ref do_core_control(long temp)
{
	...
	if (core_control_mask &&
		temp >= core_limit_temp_degC) {
		for (i = num_possible_cpus(); i > 0; i--) {
			if (!(core_control_mask & BIT(i)))
				continue;
	...
	} else if (core_control_mask && cpus_offlined &&
		temp <= (core_limit_temp_degC -
			core_temp_hysteresis_degC)) {
...
static void __ref do_freq_control(long temp)
{
	...
	} else if (temp < limit_temp_degC -
		 temp_hysteresis_degC) {
                ...
                struct kobj_attribute *attr, char *buf)
{
	...
	overtemp = min(limit_temp_degC,
			core_limit_temp_degC);

	warning = min((limit_temp_degC -
	               temp_hysteresis_degC),
		      (core_limit_temp_degC -
		       core_temp_hysteresis_degC));
...
static int __ref update_offline_cores(int val)
{
	...
	cpus_offlined = core_control_mask & val;
	if (!core_control_enabled)
		return 0;
...
int __init msm_thermal_init(void)
{
	...
	BUG_ON(sensor_id >= TSENS_MAX_SENSORS);
