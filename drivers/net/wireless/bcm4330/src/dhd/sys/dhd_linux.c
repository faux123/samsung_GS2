...
#if (defined(CONFIG_MACH_SAMSUNG_P3) && defined(CHECK_CHIP_REV)) || defined(CONFIG_MACH_C1) || defined(CONFIG_MACH_N1) || (defined(CONFIG_USA_MODEL_SGH_I717) && defined(CHECK_CHIP_REV))
	if(g_chipver == 2) {
...
#if defined(CUSTOMER_HW_SAMSUNG) && defined(CONFIG_WIFI_CONTROL_FUNC)
	printf("dhd_module_init fail error=0x%x\n", error);
	/* renove device */
	wifi_del_dev();
#endif
	return error;
