...
//static struct wake_lock voice_wake_lock;
...
#if defined(CONFIG_KOR_MODEL_SHV_E110S) || defined(CONFIG_KOR_MODEL_SHV_E120S) || defined(CONFIG_KOR_MODEL_SHV_E120K) ||defined (CONFIG_KOR_MODEL_SHV_E120L)||defined (CONFIG_USA_MODEL_SGH_T989) || defined(CONFIG_USA_MODEL_SGH_I727) || defined(CONFIG_KOR_MODEL_SHV_E160S) || defined(CONFIG_JPN_MODEL_SC_03D)
/* BEGIN: VPCM */
...
if( voice.voc_path == VOC_PATH_PASSIVE) 
{
	/* Send start vpcm and wait for response. */
...
/* BEGIN: VPCM */
	...
	pr_info(" send vpcm_volume_change_cmd, pkt size = %d\n",
				vpcm_volume_change_cmd.hdr.pkt_size);
			...
			void *private_data)
{
...
#if defined(CONFIG_KOR_MODEL_SHV_E110S) || defined(CONFIG_KOR_MODEL_SHV_E120S) || defined(CONFIG_KOR_MODEL_SHV_E120K) ||defined (CONFIG_KOR_MODEL_SHV_E120L)||defined(CONFIG_USA_MODEL_SGH_I727) || defined(CONFIG_KOR_MODEL_SHV_E160S) || defined(CONFIG_JPN_MODEL_SC_03D)
	...
	case AUDDEV_EVT_DEV_CHG_VOICE:
		//printk("%s AUDDEV_EVT_DEV_CHG_VOICE \n", __func__);
		if (v->dev_rx.enabled == VOICE_DEV_ENABLED)
	...
	case AUDDEV_EVT_DEVICE_VOL_MUTE_CHG:
		...
		//printk("%s AUDDEV_EVT_DEVICE_VOL_MUTE_CHG \n", __func__);
		if (evt_payload->voc_devinfo.dev_type == DIR_TX) {
			//printk("%s AUDDEV_EVT_DEVICE_VOL_MUTE_CHG - evt_payload->voc_devinfo.dev_type == DIR_TX\n", __func__);
			v->dev_tx.mute =
			...
			if (v->voc_state == VOC_RUN)
			{
				//printk("%s AUDDEV_EVT_DEVICE_VOL_MUTE_CHG - evt_payload->voc_devinfo.dev_type == DIR_TX v->voc_state == VOC_RUN\n", __func__);
				voice_send_mute_cmd_to_modem(v);
			}

			mutex_unlock(&v->lock);
		} else {
		//printk("%s AUDDEV_EVT_DEVICE_VOL_MUTE_CHG - evt_payload->voc_devinfo.dev_type == DIR_RX\n", __func__);
			v->dev_rx.volume = evt_payload->
				voc_vm_info.dev_vm_val.vol;

			//printk("%s AUDDEV_EVT_DEVICE_VOL_MUTE_CHG - v->dev_rx.volume =%d\n", __func__ , v->dev_rx.volume);

			mutex_lock(&v->lock);

			if (v->voc_state == VOC_RUN)
			{
			//printk("%s AUDDEV_EVT_DEVICE_VOL_MUTE_CHG - evt_payload->voc_devinfo.dev_type == DIR_RX v->voc_state == VOC_RUN\n", __func__);
	...
	case AUDDEV_EVT_REL_PENDING:
		...
		mutex_lock(&v->lock);
		//printk("%s AUDDEV_EVT_REL_PENDING \n", __func__);
...
static int __init voice_init(void)
{
	int rc = 0;
	struct voice_data *v = &voice;
	//wake_lock_init(&voice_wake_lock, WAKE_LOCK_SUSPEND, "voice_wake_lock");
	/* set default value */
