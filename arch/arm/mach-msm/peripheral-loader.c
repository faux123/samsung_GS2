...
static int load_image(struct pil_device *pil)
{
	...
	debug_printk("@@ %pS(0x%x, 0x%x)\n", pil->ops->init_image, fw->data, fw->size);
	ret = pil->ops->init_image(fw->data, fw->size);
	debug_printk("@@ %pS returned with %d\n", pil->ops->init_image, ret);
	if (ret) {
	...
	debug_printk("@@ %pS\n", pil->ops->auth_and_reset);
	ret = pil->ops->auth_and_reset();
	debug_printk("@@ %pS returned with %d\n", pil->ops->auth_and_reset, ret);
	if (ret) {
