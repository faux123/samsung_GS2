...
static void clock_showall_enabled(void)
{
	struct clk *clk;
	int cnt = 0;
...
static int clock_debug_showall_get(void *data, u64 *val)
{
	clock_showall_enabled();
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(clock_showall_fops, clock_debug_showall_get,
			NULL, "%llu\n");
...
int __init clock_debug_init(struct list_head *head)
{
	...
	clocks_ptr = head;
	(void) debugfs_create_file("showall", S_IRUGO, debugfs_base, NULL,
				&clock_showall_fops);
	return 0;
}
