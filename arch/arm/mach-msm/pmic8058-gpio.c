...
//#define PMIC_GPIO_TSP_3P3		PM8058_GPIO(16)  	/* PMIC GPIO Number 16 */
//#endif
			  ...
			  size_t count, loff_t *ppos)
{
	...
	mode = (bank[2] & PM8058_GPIO_PULL_MASK) >>  PM8058_GPIO_PULL_SHIFT;
	switch(mode) {
	case PM_GPIO_PULL_UP_30:
	case PM_GPIO_PULL_UP_1P5:
	case PM_GPIO_PULL_UP_31P5:
	case PM_GPIO_PULL_UP_1P5_30:
		len += snprintf(debug_read_buf + len, DEBUG_MAX_RW_BUF - len - 1,
			       "PULL_UP ");
		break;

	case PM_GPIO_PULL_DN:
		len += snprintf(debug_read_buf + len, DEBUG_MAX_RW_BUF - len - 1,
			       "PULL_DOWN ");
		break;

	case PM_GPIO_PULL_NO:
		len += snprintf(debug_read_buf + len, DEBUG_MAX_RW_BUF - len - 1,
			       "NO_PULL ");
		break;
	}
...
static void pm8058_gpio_rapidinit(void)
{
#if 0 //!defined(CONFIG_TARGET_LOCALE_US_ATT_REV01)
	int ret;
	ret =pm8058_gpio_set(&pm8058_gpio_chip, PMIC_GPIO_TSP_3P3, 1);   // 15->16
	if (ret) {
		pr_err("gpio write failed\n");
	}		
#endif
